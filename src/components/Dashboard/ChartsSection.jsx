...
const ChartsSection = () => {
  ...
  // On trie pour avoir les plus gros rouleurs en premier
  ...
  // On regroupe les km par nom de profil
  const ownerStats = {};
  bikes.forEach(bike => {
    // Nouvelle structure : bike.profiles contient { name: '...', avatar: '...' }
    const ownerName = bike.profiles?.name || 'Inconnu';
    if (!ownerStats[ownerName]) ownerStats[ownerName] = 0;
    ownerStats[ownerName] += 1; // On compte le nombre de vélos (ou += bike.total_km pour les km)
  });
